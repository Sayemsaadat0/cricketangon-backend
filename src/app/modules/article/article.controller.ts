...
const updateArticle = catchAsync(async (req: Request, res: Response) => {
    const articleId = Number(req.params.id);    const articleUpdates = req.body; 
    const file = req.file; 
    const updatedArticle = await ArticleService.updateArticle(articleId, articleUpdates, file);
    sendResponse(res, {
        statusCode: httpStatus.OK,
