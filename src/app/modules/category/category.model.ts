...
import { ICategory } from './category.interface';
 ...
 const createCategory = (category: ICategory): Promise<Partial<ICategory>> => {
  ...
  return new Promise((resolve, reject) => {
    connection.query(CategoryQueries.CREATE_CATEGORY, [name, image], (err, results) => {
      if (err) {
        return reject(
          new ApiError(
 ...
 const getCategoryById = (id: number): Promise<ICategory | null> => {
  return new Promise((resolve, reject) => {
    connection.query(CategoryQueries.GET_CATEGORY_BY_ID, [id], (err, results) => {
      if (err) {
        return reject(
          new ApiError(
            httpStatus.INTERNAL_SERVER_ERROR,
 ...
 const updateCategory = (id: number, categoryUpdates: Partial<ICategory>): Promise<ICategory> => {
